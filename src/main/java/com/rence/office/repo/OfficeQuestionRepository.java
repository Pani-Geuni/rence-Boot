...
public interface OfficeQuestionRepository extends JpaRepository<OfficeQuestionVO, Object> {
	...
	@Query(nativeQuery = true,
			value="select * from (select ROWNUM as rn, c.* from ( select * from question_list_view where mother_no is null and backoffice_no=?1 order by comment_no asc) c ) where rn between ?2 and ?3")
