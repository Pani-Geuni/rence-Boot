...
public class MultipleSecurityConfiguration {
	...
	public class SecurityConfig extends WebSecurityConfigurerAdapter {
		...
		// 스프링 시큐리티(Spring Seurity) 프레임워크에서 제공하는 클래스 중 하나로 비밀번호를 암호화하는 데 사용할 수 있는 메서드를
		// 가진 클래스입니다.
		@Override
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			auth.userDetailsService(backOfficeService).passwordEncoder(passwordEncoder());
		}

		@Override
		protected void configure(HttpSecurity http) throws Exception {
			/*
			 * csrf 토큰 활성화시 사용 쿠키를 생성할 때 HttpOnly 태그를 사용하면 클라이언트 스크립트가 보호된 쿠키에 액세스하는 위험을 줄일
			 * 수 있으므로 쿠키의 보안을 강화할 수 있다.
			 */
...
//         .loginPage("/login") // 해당 주소로 로그인 페이지를 호출한다.
					...
					.expiredUrl(
							"/login?error=true&exception=Have been attempted to login from a new place. or session expired") // 세션이
																																// 만료된
																																// 경우
																																// 이동
																																// 할
...
//		      return new BCryptPasswordEncoder();//- 생성자의 인자 값(verstion, strength, SecureRandom instance)을 통해서 해시의 강도를 조절할 수 있습니다.
		...
		@Override
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			auth.userDetailsService(masterService).passwordEncoder(config.passwordEncoder());
		}
		...
		protected void configure(HttpSecurity http) throws Exception {
			...
			http.csrf().disable() // csrf 토큰을 비활성화
					.authorizeRequests().antMatchers("/master/master_main/**").authenticated() // 요청 URL에 따라 접근 권한을 설정
//		         .antMatchers("/**").permitAll() // 해당 경로들은 접근을 허용
//		         .anyRequest() // 다른 모든 요청은
//		         .authenticated() // 인증된 유저만 접근을 허용
					.and().formLogin() // 로그인 폼은
//		         .loginPage("/master/login") // 해당 주소로 로그인 페이지를 호출한다.
					...
					.and().logout().logoutRequestMatcher(new AntPathRequestMatcher("/master/logout")) // 로그아웃 URL
...
//		}
		...
		@Override
		protected void configure(AuthenticationManagerBuilder auth) throws Exception {
			auth.userDetailsService(userService).passwordEncoder(config.passwordEncoder());
		}
