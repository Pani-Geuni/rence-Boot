...
public interface ReviewRepository extends JpaRepository<ReviewListView, Object>{
	...
	@Query(nativeQuery = true, value = " select * from (select rownum as rnum, review_no, review_content, review_point, TO_CHAR(review_date, 'YYYY-MM-DD HH24:MI:SS') as review_date, user_no, user_image, substr(user_name,1,1)||lpad('*',length(user_name)-1,'*') \r\n"
