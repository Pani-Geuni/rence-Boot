...
//	public List<CommentListQView> backoffice_qna_selectAll(String backoffice_no);
	...
	@Query(nativeQuery = true, value = "select * from (select rownum as rnum, comment_no, user_no, comment_content, comment_date, room_name, comment_state, room_no, backoffice_no FROM COMMENTLIST_Q_VIEW WHERE BACKOFFICE_NO=?1 order by comment_date desc) where rnum between ?2 and ?3")
